...
export async function PATCH(req, { params }) {
  try {
    ...
    if (startup.userId !== session.user.id) return NextResponse.json({ message: "Forbidden." }, { status: 403 });
    ...
    let subscriptions = [];
    try { const raw = formData.get("subscriptions"); if (raw) subscriptions = JSON.parse(raw); } catch {}

    let techStack = [];
    try { const raw = formData.get("techStack"); if (raw) techStack = JSON.parse(raw); } catch {}
    ...
    if (forSale && !askingPrice) {
      return NextResponse.json({ message: "Please provide an asking price." }, { status: 400 });
    }
    if (forSale && seekingCofounder) {
      return NextResponse.json({ message: "Cannot be for sale and seeking co-founder simultaneously." }, { status: 400 });
    }
    ...
    await db.collection("startups").updateOne(
      ...
      {
        $set: {
          name, url, productType, category, description, logoUrl, logoPublicId,
          subscriptions, techStack, forSale, askingPrice: forSale ? askingPrice : null,
          seekingCofounder, updatedAt: new Date(),
        },
      }
    );
